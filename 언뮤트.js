...
module.exports = {
    ...
    async execute(message,args){
        ...
        if(!rolealive) return message.reply(`서버에 뮤트 역할이 등록되지 않았어요. ${config.prefix}뮤트생성으로 역할을 생성해주세요 !`)
        const role = message.guild.roles.cache.find(r=> r.id == `${rolealive.roleid}`)
        if(!role) return message.reply(`뮤트 역할이 삭제되거나 오류가 발생했어요. ${config.prefix}뮤트생성으로 역할을 다시 생성해주세요 !`)
