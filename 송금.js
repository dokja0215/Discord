...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!sk) return message.reply("**통장을 먼저 개설해주세요. ( !통장 )**") // 데이터베이스에서 userid 가 메세지를 보낸사람의 id인것이 없다면 리턴
        if (!tkdeoqkd) return message.reply("**송금할 대상이 통장을 개설하지 않았습니다**") // 위에꺼랑 비슷 (데이터베이스에서 멘션한 유저를 찾을 수 없으면 리턴)
