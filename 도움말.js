...
module.exports = {
    ...
    async execute(message) {
        const embed = new (require("discord.js")).MessageEmbed()
            .setTitle(`명령어 목록 (${config.prefix})`)
            .setColor("BLUE")
            .setThumbnail(message.author.displayAvatarURL())
            .setTimestamp()
        const slash = new (require("discord.js")).MessageEmbed()
            .setTitle("슬래쉬 명령어 목록 (/)")
            .setColor("GREEN")
            .setThumbnail(message.author.displayAvatarURL())
            .setTimestamp()
        ...
        for (const file of commands) {
            ...
            embed.addFields({ name: `${command.name}`, value: `${command.description || "없음"}`, inline: true })
        }
        const slashcommands = readdirSync(path.resolve(__dirname, `../slashcommands`)).filter(file => file.endsWith(".js"))
        for (const file of slashcommands) {
            const command = require(`../slashcommands/${file}`)
            slash.addFields({ name: `${command.name}`, value: `${command.description || "없음"}`, inline: true })
        }
        const menu = new (require("discord.js")).MessageSelectMenu()
            ...
            .addOptions([
                ...
                { label: "슬래쉬 명령어", description: "/를 입력해서 사용하는 명령어", value: "슬래쉬 명령어" }
            ])
        ...
        collector.on("collect", collected => {
            ...
            if (value == "슬래쉬 명령어") {
                return collected.update({ content: " ", embeds: [slash], components: []})
            }
        })
    }
}